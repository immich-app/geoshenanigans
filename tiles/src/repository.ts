...
export class CloudflareD1Repository implements IDatabaseRepository {
  constructor(private databases: { [key: string]: D1Database}, private deploymentKey: string, private metrics: IMetricsRepository) {}
  ...
  async query(query: string, ...values: unknown[]) {
    ...
    const { key: databaseKey, resp } = await Promise.race(
      Object.entries(this.databases).map(async ([key, database]) => {
        const resp = await database.prepare(query)
          .bind(...values)
          .run();
        return { key, resp };
      })
